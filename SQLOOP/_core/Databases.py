...
class Database(metaclass=DatabaseMeta):
	...
	def CURRENT_TABLES_HASH(self) -> int:
		return int.from_bytes(hashlib.md5(whitespacePattern.sub(" ", "; ".join(sorted(map(sql, self.tables)))).encode("utf-8")).hexdigest(), base=16)
	@ClassProperty
	def CURRENT_INDEXES_HASH(self) -> int:
		return int.from_bytes(hashlib.md5(whitespacePattern.sub(" ", "; ".join(sorted(map(sql, self.indexes)))).encode("utf-8")).hexdigest(), base=16)
	...
	def indexesHash(self) -> int:
		...
		return int.from_bytes(hashlib.md5(whitespacePattern.sub(" ", "; ".join(sorted(map(lambda s:tableCreationCommand.sub("",s), self(SELECT (SQL) - FROM - SQLITE_MASTER - WHERE (type='index')))))).encode("utf-8")).hexdigest(), base=16)

	@property
	def tablesHash(self) -> int:
		"""hash of the original SQL text that created all tables in the database."""
		return int.from_bytes(hashlib.md5(whitespacePattern.sub(" ", "; ".join(sorted(map(lambda s:tableCreationCommand.sub("",s), self(SELECT (SQL) - FROM (SQLITE_MASTER) - WHERE (type='table')))))).encode("utf-8")).hexdigest(), base=16)
