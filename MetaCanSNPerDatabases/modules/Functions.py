...
def updateFromLegacy(database : DatabaseWriter, refDir=""):
	...
	database._connection.execute("COMMIT;")
	database._connection.execute("BEGIN TRANSACTION;")
	database._connection.execute(f"INSERT INTO {TABLE_NAME_TREE} ({TREE_COLUMN_PARENT}, {TREE_COLUMN_CHILD}, {TREE_COLUMN_NAME}) SELECT tree_old.parent-1, null, nodes.name FROM tree_old, nodes WHERE tree_old.child = nodes.id ORDER BY tree_old.child ASC;")
	database._connection.execute("COMMIT;")
	database._connection.execute("BEGIN TRANSACTION;")
